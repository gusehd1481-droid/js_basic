...
//함수준비
function func_z(){
    z = 5;    //전역변수 z 값 5대입 위치 = 지역스코프(전역에서 인식못함)
    func_y(); //지역 변수  y10 값 인식을 위해 y10을 가지고 있는 함수 호출
    console.log(x+y); //11
}
function func_y(){
    y = 10;   //전역변수 y 값 10대입 위치 = 지역스코프(전역에서 인식못함)
...
console.log(snack); // undefined (roomA호출후)
roomB();
console.log(snack);
//함수 준비
function roomA(){ 
...
function roomB(){//함수 블록 스코프 안
    let milk = '우유'
    console.log(`B방에서 먹는 간식${snack}, ${milk}`); //전역변수 호출(roomB를 호출해야 가능)
}
...
//함수준비
function school(meals='김밥'){ //매개변수는 let, const 없이도 알아서 변수를 인식함.
    //let meals = '부대찌개';
    console.log(`오늘의 급식메뉴 : ${meals}`)
}
...
//함수 호출
//house('조약돌');
house(); //매개변수의 기본값이 있을 때 호출괄호는 비워둔다.
house('빵 부스러기');
//함수 준비
function house(fall='조약돌'){
    let story = '';
    story += `헨젤은 ${fall}를(을) 떨어뜨리면서 숲으로 갔습니다.`;
    story += `계모가 떠나고`;
    story += `헨젤은 ${fall}를(을) 따라서 집으로 돌아갔습니다.`;
    console.log(story);
}
...
cart('한라봉 3kg', 25900, 1);
cart('치아바타 샌드위치', 11800, 2);
function cart(product, price, num){
    let item = ''; 
    item += `상품명 : ${product}`;
...
coffee('따듯한', 2);
coffee(undefined,menuAll[1],);
coffee('따듯한','녹차라떼', menuAll[2]);
coffee(undefined,2,2);
function coffee(temp='차가운', menu, num=1){
    console.log(`${temp} ${menuAll[1]} ${num}잔 주문완료 되었습니다.`);
}
