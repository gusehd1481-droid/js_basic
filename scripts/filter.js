...
let filterListShowHide = false; //초기값 숨김 나중에 true로 바꿔줘야 하기 때문에 let으로 생성 (js상태 구분용)
console.log(filterView,filterList);
// 필터 목록 초기 숨기기 (객체.속성.속성 = 값;)
filterList.style.display = 'none'; //(css용)

// 필터 보기를 클릭 시
filterView.addEventListener('click', function(){
    // 필터가 숨겨진 (false) 상태면 -> 보이기
    //if(filterList == style.display = 'none'){} (X) css를 js로 쓸 수 없음.
    if(filterListShowHide == false){
        filterList.style.display = 'flex';
        //filterListShowHide = true;
        //위 코드 해석
        //flex로 보이는 상태가 됐으므로 js의 상태변수도 참(true)으로 대입해서 추적가능하게 만들기
...
for(let f of filterA){
    console.log(f) //for 문으로 반복개별출력 index 필요없음
    f.addEventListener('click', function(e){
        ...
        for(let list of filterContents){
            ...
            if(f.dataset.name == 'all'){list.style.display = 'flex';}
            //클릭한 f가 과일이면 -> data-name="fruit" 와 일치하는 list 찾아서보기
            ...
            //클릭한 f가 채소 -> data-name="veggi" 와 일치하는 list 찾아서보기
        }
    })
}