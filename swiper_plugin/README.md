...
## Swiper 작성 준비
0. `head` 태그안 `title` 아래 swiper사용을 위한 CDN 연결
    `</title>  **CDN연결파일들** reset, css, 개별js 등 연결 마무리`
    **CDN 연결**
...
2. 1번에서 생성한 태그에 대상 구분용의 별도 클래스 따로 생성
    `div.swiper.개별클래스명` -> `<div class="swiper 개별클래스명">`
    `div.swiper-slide.slide번호` -> `<div class="swiper-slide slide번호">`
3. 프로젝트 디자인에 따라 필요한 추가태그들을 작성
    `div.swiper-slide` 안에 `a, h, p, form, table, div 등..`
4. CSS 선택자 준비 (개별, 공통용 모두)
    * **주의사항**
    * `swiper-wrapper, swiper-slide`
        `display, width, height, position` 적용하지 않기 
    * `swiper` 제일 바깥쪽 swiper 시작태그한테만 크기적용
    * `swiper-slide` 안 3번 목적으로 만든 태그들은 속성 자유 h,p 등 추가태그
...
* `loop:true,` 반복을 묻는 옵션, true(반복함)
* `effect:fade,` 서서히 사라지는 이펙트, `cube`, `flip` **`fade` 사용시 direction 적용을 해제해야함**
* `direction:horizontal,` 슬라이드 방향 설정, `수평 기본값`, `수직(vertical)` 
* `mousewheel:true,` 마우스휠 가능 옵션, true 시 마우스휠로 슬라이드 조작 가능
* `speed:1000,` 슬라이드가 넘겨지는 속도 설정 
* `autoplay:{옵션,값},` 자동재생, autoplay에 대한 옵션이 많아 {}안에 작성
    * `delay:1000,` 재생 딜레이 설정, ms단위 1000 -> 1s
    * `pauseOnMouseEnter:true,` 마우스가 슬라이드 안으로 들어올 시 정지 옵션
    * `disableOnInteraction:false,` 마우스 상호작용 후 정지 or 동작 선택 옵션, `움직임(false)`
...
### swiper 주의사항
* `swiper-wrapper`, `swiper-slide`는 개별 크기 입력하지 않기 
...
* 태그위치 `div.swiper` 두번째 자식 위치로 생성
### 기존 Swiper-slide 외부 바깥쪽으로 안겹치는 안겹치는 디자인을 하고 싶을 때
* 태그위치 `div.swiper` 다음 형제 위치로 생성
* `div.swiper` 와 생성된 선택태그를 묶어주는 추가 태그 생성(디자인에 따라 다름)
### 교차 여부에 따라 위치를 다르게 해야하는 이유
* `div.swiper`에 기본 css에 `overflow:hidden` 있으므로 그 값을 `visible` 로 바꾸면 선택요소 뿐 아닌 `swiper-slide` 들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.
## Swiper 안에 추가 Swiper 생성 시 안쪽 Swiper 마우스드래그 안될 때 해결 방법
* 안쪽 Swiper js 옵션:값으로 아래와 같이 작성
    * `nested:true,`
    * 부모 swiper와 겹쳐 있을 때 드래그 충돌방지 옵션:값,
## Swiper Custom function
1. `renderFranction:function(){}`
    * pagination:{} 안에 작성해야함.
...
## Swiper-slide 개별 속성
1. `slidesPerView` : 하나의 화면 기준 보이는 슬라이드 개수 **총 슬라이드 개수보단 작아야 함.**
2. `slidesPerGroup` : 슬라이드 한번에 전환되는 개수 (기본 1개) **하나의 화면 기준 보이는 개수보다 많을 수 없음.**